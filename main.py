...
app = FastAPI(
    ...
    description="Now includes reading time, paragraph count, and word count.",
    version="8.1.0",
)
...
def _add_article_stats(full_text: str) -> dict:
...
def _scrape_substack_article(soup: BeautifulSoup) -> dict:
    try:
        ...
        full_text = '\n\n'.join(text_blocks)
        stats = _add_article_stats(full_text)
